-- Run this first. Creates types, domains, sequences, tables, functions, aggregates, and views.
-- Data is loaded by the later seed files.
-- Fixed ordering: helper functions are created before functions that call them.

-- Run this first. Creates types, functions, sequences, tables, views, indexes, and primary/unique constraints where applicable.

-- Supabase-ready dvdrental schema + seed data
-- Creates tables/sequences/types if they do not exist, loads the sample data,
-- and adds keys, indexes, and triggers. Safe to paste into the Supabase SQL editor.
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'year') THEN
...
-- Functions and aggregates: created after tables so referenced relations already exist.
...
CREATE OR REPLACE FUNCTION public.get_customer_balance(p_customer_id integer, p_effective_date timestamp without time zone) RETURNS numeric
    LANGUAGE plpgsql
    AS $$
...
-- Views: created after all base tables and helper functions exist.
